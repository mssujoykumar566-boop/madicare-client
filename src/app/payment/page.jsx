...
function PaymentContent() {
      ...
      const fetchAppt = async () => {
        try {
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_SERVER_URL}/api/appointments/${appointmentId}`,
          );
          if (res.ok) {
  ...
  const handleStripePayment = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/appointments/${appointmentId}/pay`,
        {
          method: "PATCH",
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-slate-500 text-xs font-semibold mt-1">
                Processed securely using Stripe Checkout
              </p>
            </div>
          ...
          <form onSubmit={handleStripePayment} className="space-y-5">
            <div className="bg-gradient-to-br from-teal-500 to-cyan-600 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden h-44 flex flex-col justify-between">
              ...
              <div className="flex justify-between items-end text-xs">
                <div>
                  <span className="opacity-60 text-[9px] block uppercase">
                    Card Holder
                  </span>
            ...
            >
              ...
              {payLoading
                ? "Authorizing Stripe Checkout..."
                : `Pay ${appointment.fee} BDT`}
            </button>
          </form>
        </div>
      </div>
